...
3. Fetches AI summaries and writes `.md` files under `{vault}/Plaud/{folder}/{year}/` (folder tags mirrored; untagged → `Без категории`).
...
| Output | Path |
|--------|------|
| Summaries | `{PLAUD_EXPORT_ROOT or vault}/Plaud/{folder}/{YYYY}/YYYY-MM-DD - {title}.md` (`Без категории` when no Plaud folder) |
