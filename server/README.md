...
- 📁 **Файлы**:
  - **🌳 Дерево синка** — папки из `sync-index.json` (с учётом `folderSegment` при `PLAUD_MIRROR_FOLDERS=true`); внутри
    папки — постраничный список записей с номерами `1 -` … `30 -`;
  - отправьте боту **цифру** (1–30 на текущей странице), чтобы получить соответствующий `.md` как документ (если файл
    уже есть на диске после синка);
  - **📊 Сводка vault** — число `.md`, размер, последние 10 файлов по дате изменения;
- ⚙️ Настройки расписания — выбор интервала (60 / 120 / 240 / 480 минут) и переключатель уведомлений автосинка (🔕 по
  умолчанию выкл — автосинк работает тихо; 🔔 вкл — бот пришлёт «🕒 Автозапуск синка…» в чат). Оба значения хранятся в
  `server/.data/bot-settings.json`. Ручной 🔄 всегда отвечает в чат вне зависимости от тумблера;
- ℹ️ Помощь — справочник по командам.

Индекс sync (`syncCore.js`): при смене только папки Plaud (`folderSegment`), названия или имени файла без изменения хеша
саммари запись помечается как обновлённая метаданными — файл на диске **переименовывается/переносится** без повторной
загрузки с API.
...
| Thinking reveal                  | `messageAnimator.js`, `streamingDelivery.js` | Нативное облачко «Думаю…» (пустой `sendMessageDraft` / `<tg-thinking>`) + один кадр полного текста перед `sendMessage` / `editMessageText`; порог ~60 символов; короткие toast и `animate: false` — мгновенно |
| HTML blockquote                  | `htmlFormat.js`                              | Длинная справка; retry без blockquote при 400                                                                                                                                                                 |
| Message effect                   | `telegramVisual.js`                          | ✨ после ручного успеха в личке                                                                                                                                                                               |
