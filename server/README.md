...
- 📁 **Файлы**:
  - **🌳 Дерево синка** — папки из `sync-index.json` (с учётом `folderSegment` при `PLAUD_MIRROR_FOLDERS=true`); внутри папки — постраничный список записей с номерами `:one:` … `:twenty:`;
  - отправьте боту **цифру** (1–20 на текущей странице), чтобы получить соответствующий `.md` как документ (если файл уже есть на диске после синка);
