/** Pure session interpretation. One immutable storage view per read. */
...
/** @param {{localStorage: Record<string, string>, sessionStorage: Record<string, string>}} snapshot */
export function createSessionReader(snapshot) {
  ...
  function extractWorkspaceListFromValue(value, depth = 0, seen = new Set()) {
    ...
    for (const [key, child] of entries) {
      if (!WORKSPACE_LIST_KEY_RE.test(key) && typeof child !== "object")
        continue;
   ...
   */
  function describePlaudSessionStorage(origin = "") {
  ...
  function resolveSession() {
    const userTokenCandidate = findUserTokenCandidate();
  ...
  return {
    ...
    resolveSession,
    hasUserToken: () => !!findUserTokenCandidate().token,
  };
}
