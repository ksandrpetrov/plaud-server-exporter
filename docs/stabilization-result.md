...
- **Tests.** 67 server + 14 extension tests; all green with `npm test`, `npm run lint`, `npm run verify`.
...
```bash
npm test                 # 67 server tests
npm run lint
npm run verify
npm run test:submodule   # 14 extension tests
```
